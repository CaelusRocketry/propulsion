...
C
C ----- modified by A. Miyoshi (UT) Oct. 11, 2014 -----
C  - added .csv file output
C  - added double quatations to species names
C    (Dec. 2, 2016)
C -----------------------------------------------------
C***********************************************************************
...
C INITIAL ESTIMATES
 ...
 120      CONTINUE
        ...
        IF ( Nplt.GT.0 ) THEN
          ...
          WRITE (IOPLT,99009) (Pltvar(j),j=1,Nplt)
C ----- modified by A. Miyoshi (UT) Oct. 11, 2014 -----
          OPEN (IOCSV,FILE=Cfile,FORM='formatted')
          WRITE (IOCSV,99099) (Pltvar(j),j=1,Nplt)
          DO i = 1,Iplt
            WRITE (IOCSV,99095) (Pltout(i,j),j=1,Nplt)
          ENDDO
C -----------------------------------------------------
        ENDIF
      ENDIF
...
C ----- modified by A. Miyoshi (UT) Oct. 11, 2014 -----
99095 FORMAT (1x,1p,20(E13.5:', '))
C    (Dec. 2, 2016)
C99099 FORMAT (1x,20(A12:','))
99099 FORMAT (20('"'A12'"':','))
C -----------------------------------------------------
      END
      BLOCKDATA 
